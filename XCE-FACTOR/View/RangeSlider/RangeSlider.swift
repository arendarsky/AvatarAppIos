...
//  RangeSlider
//
//  Created by Владислав on 30.01.2020.
...
@IBDesignable open class RangeSlider: UIControl {
  ...
  @IBInspectable open var minimumDistance: Double = 0.0 {
    ...
    ///The maximum difference in value between the Knobs. `0.0` is both default and disabled
    @IBInspectable open var maximumDistance: Double = 0.0 {
      didSet {
        updateTrackLayerFrameAndKnobPositions()
      }
    }
 ...
 */
        let labelText = String(format: "%02i:%02i", minutes, seconds)
        return labelText
    }
  ...
  override open func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
    ...
    if lowerKnob.highlighted && upperKnob.highlighted {
        ...
        if (deltaValue > 0) {
            ...
            upperValue = boundValue(newUpperValue, toLowerValue: (lowerValue + gap), upperValue: maximumValue)
            let newLowerValue = lowerValue + deltaValue
            lowerValue = boundValue(newLowerValue, toLowerValue: minimumValue, upperValue: (upperValue - gap))
        } else {
            let newLowerValue = lowerValue + deltaValue
            lowerValue = boundValue(newLowerValue, toLowerValue: minimumValue, upperValue: (upperValue - gap))
            let newUpperValue = upperValue + deltaValue
      ...
      var newLowerValue = lowerValue + deltaValue
        if upperValue - newLowerValue > maximumDistance {
            newLowerValue = upperValue - maximumDistance
        }
      ...
      var newUpperValue = upperValue + deltaValue
        if newUpperValue - lowerValue > maximumDistance {
            newUpperValue = maximumDistance + lowerValue
        }
